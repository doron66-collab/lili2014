...
%% ── Metadata ─────────────────────────────────────────────────────────────────
\hypersetup{
  pdftitle  = {A Governed QC-AI-HPC Architecture for Deciding When Quantum Computation Is Warranted in Non-Druggable Oncology},
...
%% ── Title block ──────────────────────────────────────────────────────────────
\begin{center}
{\Large\bfseries A Governed QC-AI-HPC Architecture for Deciding When\\
Quantum Computation Is Warranted in Non-Druggable Oncology}\\[10pt]
...
{\small Working draft, September 2026 (revised from the May 2026 version)}\\
{\small Intended arXiv category: cs.ET \quad|\quad Not yet submitted}\\
...
Regulated quantum-oncology computation is an information-systems problem before it is a
computational-chemistry problem.  The first question is not how to run a quantum algorithm
but whether a given target warrants one.  This paper presents a governed QC-AI-HPC
orchestration architecture that adjudicates, per mutation target, whether quantum computation
is warranted, and that records every step of that decision in a nine-element P1-P9 provenance
record aligned with the audit-trail logic of FDA~21~CFR~Part~11.

The architecture has three layers.  Layer~1 is the quantum-kernel layer, anchored by
sample-based quantum diagonalization (SqDRIFT) on IBM Heron-class hardware.  Layer~2 is the
hybrid-substrate layer, covering classical preprocessing, GPU/HPC services, AI calibration,
and emerging learned quantum-error-correction decoders.  Layer~3 is the governed
orchestration and provenance layer.  Its central component is a two-method classifier:
DMRG and SHCI, two classical methods with independent failure modes, run on the same active
space, and the platform records the verdict of each method and any disagreement between them.

The classifier has now been exercised on real targets.  Every real biological target measured
to date, including TP53~C275F at active spaces of 36 and 48 electrons, is classically
tractable (Class~B).  The same classifier returns the opposite verdict (Class~A) for a
declared model-compound proxy of a [2Fe-2S] redox center, so the Class~B findings are
measurements rather than a default.  Small pipeline test jobs (4 to 8 qubits) also ran on
three IBM Heron-class devices in July and August 2026; they validate the pipeline, not
chemical accuracy.  A design-science evaluation protocol is proposed.  No energy result in
this paper should be interpreted as a quantum-computed molecular binding energy.
...
precision oncology; classical-tractability adjudication; DMRG; SHCI; TP53; NSCLC; SqDRIFT; IBM Heron; NVIDIA NVQLink; FDA~21~CFR~Part~11;
provenance.
...
\clearpage
\section*{Status Update, September 2026}

This version revises the May 2026 manuscript.  Four developments since May change how the
paper should be read.

\textbf{Reframing.}  The contribution is now stated as adjudication: the platform decides,
per target, whether quantum computation is warranted, instead of assuming that it
is~\cite{cohen2026proposal}.

\textbf{Two-method classification.}  The escalation rule of Section~5 now runs DMRG and SHCI
in parallel on the same active space.  The two methods fail in independent ways, so agreement
between them is stronger evidence than either result alone, and a disagreement is flagged
instead of hidden.

\textbf{Measured classifications.}  Eight real targets have been classified by DMRG, and all
are Class~B: TP53~C275F, R175H, R282W, G245S, and R249S; SETD2~R1625C; KEAP1~G333C; and
STK11~D194N.  C275F was measured at 36 and 48 active electrons (S$_\mathrm{max}$ = 0.25 and
0.35), and SHCI agrees on its class.  The earlier figure of about 44 electrons and 88 qubits
for C275F was an architectural estimate without a verifiable derivation and has been
withdrawn.  An early provisional Class~A reading for R175H did not survive a warm-started
re-run.  The Class~A designations of the May 2026 version are therefore superseded.  Size
figures for targets not yet measured remain pre-measurement estimates and are labeled as such.

\textbf{Hardware and evidence discipline.}  Between July and August 2026, pipeline test jobs
at CAS(2,2) to CAS(4,4), on 4 to 8 qubits, ran on three IBM Heron-class devices (Kingston,
Marrakesh, and Fez).  On September 23, 2026, a first wild-type-versus-mutant energy difference
was retracted after a check showed that the mutation had not been applied to the input
structure.  The retraction is recorded in the provenance trail rather than silently corrected.

Sections~3, 5, 7, and 10 have been revised to match these findings.
...
\begin{table}[H]
\centering
\caption{Workload tiers used by the orchestration architecture.  Rows marked ``measured'' reflect September 2026 DMRG classifications; rows marked ``estimate'' are pre-measurement size estimates.}
\label{tab:tiers}
\small
...
Reference molecular benchmark & 32 & 100 &
  External SqDRIFT/Heron precedent (IBM Research 2026; arXiv:2603.08696) \\
C275F active site (measured) & 18-48 & 24-56 &
  AVAS-tested range.  DMRG Class~B at 36e and 48e (S$_\mathrm{max}$ = 0.25, 0.35); SHCI agrees.  Within Heron~r3 capacity (156 qubits) \\
KEAP1 G333C site (measured) & 30-72 & 32-74 &
  AVAS-tested range.  DMRG Class~B (S$_\mathrm{max}$ = 0.22 at 72e).  Within Heron~r3 capacity \\
KEAP1 full Nrf2 interface (estimate) & $\sim$155 & $\sim$310 &
  Pre-measurement size estimate; PDB:~2FLU (19 interface residues) \\
STK11/LKB1 full ATP pocket (estimate) & $\sim$152 & $\sim$304 &
  Pre-measurement size estimate; PDB:~2WTK (8\,\AA{} shell).  STK11~D194N measured Class~B at its own site \\
\bottomrule
\end{tabular}
\end{table}
...
The platform was designed around NSCLC mutation targets selected on three criteria:
(1)~clinical significance in NSCLC specifically, (2)~absence of an approved targeted therapy,
and (3)~an active space large enough that classical tractability cannot be assumed in advance.
Hardware feasibility on current quantum computers was deliberately \emph{not} a selection
criterion, because the architecture must be mutation-agnostic.  STK11 and KEAP1 targets were
selected for their clinical urgency in NSCLC: STK11/LKB1 loss-of-function occurs in
$\sim$20\% of NSCLC adenocarcinomas and is a driver of immunotherapy (PD-1/PD-L1) resistance,
and KEAP1 mutations co-occur in $\sim$10-15\% of NSCLC cases.

The May 2026 version of this paper designated TP53~C275F and three STK11 targets as Class~A
(quantum-necessary).  Measurement has replaced that designation: every real target classified
so far, including C275F at 36 and 48 active electrons, is Class~B.  C275F remains the primary
stress-test case for two reasons.  It anchors the work in a real patient report, and its
measured active space (18 to 48 electrons, 24 to 56 qubits across tested criteria) sits within
IBM Heron~r3 capacity, so Phase~3B can demonstrate the governed pipeline at hardware scale.
On current evidence, that demonstration tests the architecture; it does not answer a question
that classical computation cannot already answer.
...
this layer is anchored by SqDRIFT on IBM Heron~r3.  SqDRIFT is a sample-based quantum
diagonalization method: the quantum processor samples the dominant electronic configurations,
and a classical solver diagonalizes the Hamiltonian projected onto that sampled subspace.  It
matters because the target problem class is fermionic-Hamiltonian ground-state estimation at
active-space sizes beyond the classical exact ceiling.  IBM's
public materials (IBM Research; arXiv:2603.08696, and the SqDRIFT method paper
arXiv:2508.02578) describe a 2026 quantum-computing contribution to the discovery of the
half-M\"obius molecule C\textsubscript{13}Cl\textsubscript{2}, in which SqDRIFT on 100~qubits
explores a 32-electron active space beyond an approximately 18-electron exact-classical
...
formulations~\cite{abrams1999,aspuru2005,bennett1997}.  No formal classical-quantum
separation is claimed: ground-state finding is QMA-hard in the general case, and
containment in BQP is conditional on preparing an initial state with sufficient overlap on
the true ground state.  Lee et al.~\cite{lee2023} argue that these caveats erode the case
for exponential quantum advantage across chemical space; that critique is accepted here in
the general case and delimits the present claim to a \emph{practical} quantum algorithm in
the selected operational regime, not a proof of exponential advantage.  If a classical method later demonstrates
...
The mutations targeted by this dissertation differ from KRAS in one respect: most have no
resolved drug-binding pocket and no prior computational characterisation at chemical accuracy.
The escalation rule in Table~\ref{tab:escalation} therefore does not assume that quantum
simulation is required for them; it tests that assumption per target, with two independent
classical methods.  To date the test has returned Class~B for every real target measured,
which means classical tensor-network methods still converge at those sizes.  The Vakili et
al.\ KRAS workflow would also fail condition C1 and would route to Layer~2 rather than to the
quantum kernel.
...
Phase~3A of the prototype executes live 4-qubit VQE circuits on a PennyLane
\texttt{default.qubit} noiseless simulator.  Jordan-Wigner Hamiltonians are derived from
real PySCF CASSCF(2,2)/STO-3G quantum chemistry calculations on mutation-specific sidechain
model compounds (toluene for Phe275, methanethiol for Cys, acetic acid for Asp194, etc.),
producing 27 Pauli terms per target stored in a pre-computed Hamiltonian library.  At
runtime, an AllSinglesDoubles UCCSD ansatz with Hartree-Fock initial state $|1100\rangle$
is optimised over 80 Adam steps, converging to within $10^{-5}$~H$_\mathrm{a}$ of the
CASSCF-exact ground state by proof for the CAS(2e,2o) active space.  Every simulation run
exercises the complete pipeline - NGS ingestion, Hamiltonian construction, live circuit
execution, provenance assembly, cryptographic sealing, and database storage - and produces
a P1-P9 provenance record with a unique P8 SHA-256 seal, demonstrating that the
governance schema correctly distinguishes between mutation-specific ground-state energies.
Phase~3B scales this minimal active space toward the measured C275F range (up to 48 active
electrons) on IBM Heron~r3 via SqDRIFT, contingent on IBM Quantum Network access.  Between
July and August 2026, small test jobs (CAS(2,2) to CAS(4,4), 4 to 8 qubits) already ran
through the same pipeline on three IBM Heron-class devices.  They validate the pipeline, not
chemical accuracy.

\subsection*{The Transitional Window: Situating SqDRIFT and Heron~r3 Within IBM's Fault-Tolerant Roadmap}

The selection of IBM Heron~r3 as the quantum hardware substrate and SqDRIFT as the
molecular simulation algorithm reflects a deliberate positioning within a well-defined and
time-bounded technological window.  IBM's publicly disclosed roadmap - supported by a
\$10+~billion capital commitment and a \$1~billion CHIPS Act award - establishes a clear
architectural trajectory away from noisy intermediate-scale quantum (NISQ) devices toward
fault-tolerant systems based on quantum low-density parity check (qLDPC) error correction.
The milestones on this trajectory are specific: the Kookaburra processor (2026) introduces
modular quantum memory, Cockatoo (2027) entangles multi-chip configurations, and Starling
(2029) delivers the first large-scale fault-tolerant system at 200 logical qubits executing
100 million quantum gates.

This roadmap does not diminish the contribution of the present work - it contextualizes and
justifies it.  Heron~r3, as the current-generation superconducting processor, and SqDRIFT,
as a leading NISQ-era sample-based quantum diagonalization protocol whose stochastic state
sampling inherits qDRIFT's formally bounded error scaling, represent the state of the art during precisely the window in which clinically
relevant quantum molecular simulation first becomes computationally tractable.  IBM's own
March 2026 demonstration applied SqDRIFT on 100 qubits to a 32-electron active space of
C\textsubscript{13}Cl\textsubscript{2} - a benchmark that for the first
time brings the active-space requirements of NSCLC tumor-suppressor mutations within reach
of available hardware.

The significance of this window extends beyond the chemistry.  The transition to
fault-tolerant quantum computing will not resolve - and in fact will intensify - the
information systems challenges that constitute the primary contribution of this
dissertation.  As IBM scales toward Starling (2029) and the Blue~Jay quantum-centric
supercomputer (2,000 logical qubits, 2033), targeting drug discovery as an explicit
commercial vertical, the regulatory reconstruction problem becomes more acute, not less.
Probabilistic, learned quantum outputs will be generated at greater scale and complexity,
yet no established framework exists for governing their audit trails, ensuring
reproducibility, or satisfying FDA 21~CFR Part~11 requirements in a clinical submission
context.  The pipeline architecture, P1-P9 provenance schema, and bifurcation-aware
orchestration developed in this dissertation are designed precisely for this trajectory:
they establish the IS governance substrate that fault-tolerant quantum drug discovery
workflows will require, built and validated during the window in which the underlying
quantum computation first becomes feasible.

\textit{The present work therefore occupies a strategically necessary position: it is
neither premature nor transitional.  It is foundational.}
...
EWF-TrimSQD at chemical accuracy~\cite{merz2026protein}.  C275F's measured active
space (up to 48 electrons) falls within this demonstrated range.  The architectural contribution of this dissertation is therefore validated against
hardware that can execute Phase~3B; the remaining step is institutional IBM Quantum Network
access, not algorithmic or hardware feasibility.  Because every real target measured so far
is Class~B, Phase~3B would demonstrate the governed pipeline at hardware scale; on current
evidence it would not demonstrate quantum necessity.
...
\bibitem{lee2023}
S.~Lee, J.~Lee, H.~Zhai, Y.~Tong, A.~M. Dalzell, A.~Kumar, P.~Helms, J.~Gray, Z.-H. Cui, W.~Liu, M.~Kastoryano, R.~Babbush, J.~Preskill, D.~R. Reichman, E.~T. Campbell, E.~F. Valeev, L.~Lin, and G.~K.-L. Chan.
Evaluating the evidence for exponential quantum advantage in ground-state quantum chemistry.
\textit{Nature Communications}, 14(1):1952, 2023.
\doi{10.1038/s41467-023-37587-6}.

\bibitem{halfmobius2026}
IBM Research and collaborators.
Exploring pathways towards quantum advantage in quantum chemistry: the case of a molecule with half-M\"{o}bius topology.
