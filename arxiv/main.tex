...
on a PennyLane \texttt{default.qubit} simulator (Phase~3A) executing real 4-qubit VQE circuits
with PySCF-derived Jordan--Wigner Hamiltonians (CAS(2e,2o)/STO-3G, AllSinglesDoubles UCCSD ansatz,
80 Adam optimisation steps); IBM Heron~r3 hardware execution with full molecular
...
STK11~F354L local site ($\sim$48 electrons, $\sim$96 qubits; AlphaFold Q15831) is a
near-term exception: only 2 qubits beyond the demonstrated ceiling and within IBM Heron
r3 hardware capacity (156 qubits), making local Phase~3B executable without
fault-tolerant hardware.  The remaining five KEAP1 and STK11 targets require 152--310
qubits (Table~\ref{tab:tiers}) and are designated fault-tolerant QPU targets
($\sim$2030+).  Their inclusion in the platform
...
framework apply whether the backend is a 4-qubit PennyLane simulator today or a
...
prediction.  Quantum simulation is therefore not an enhancement but the
approach this artifact targets for that regime: once exact classical treatments
(CCSD(T)/CASSCF) become intractable at these active-space sizes and lower-cost
methods introduce sign-inverting error, it is the computational path this work is
built to govern and orchestrate toward a physically grounded
...
Phase~3B replaces this minimal active space with the full 24e/48q local site and 44e/88q
complete active site on IBM Heron~r3 via sqDRIFT.
