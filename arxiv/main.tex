...
Reference molecular benchmark & 32 & 100 &
  External SqDRIFT/Heron precedent for frontier electronic-structure simulation \\
C275F local binding site & $\sim$24 & $\sim$50--60 &
  Proof-of-concept workflow, simulator validation, near-term quantum-kernel test \\
C275F full binding site  & $\sim$44 & $\sim$88 &
  Within demonstrated hardware range: Merz et al.~\cite{merz2026protein} used 94 qubits on IBM Heron~r2 for 12,635-atom protein-ligand systems (May 2026); Phase~3B execution pending IBM Quantum Network access \\
\bottomrule
\end{tabular}
\end{table}
...
The platform as implemented addresses eight NSCLC mutation targets: TP53~C275F, TP53~Y220C,
KEAP1~LOF, KEAP1~G333C, KEAP1~R320Q, STK11~LKB1 (LOF), STK11~F354L, and STK11~D194N.
All eight carry BQP-relevant active spaces (Class~A or Class~B under the escalation rule
of Section~5); mutations whose active spaces remain tractable by classical
density-functional methods (Class~C) are excluded from quantum routing at the
BQP-classification stage.  C275F is used as the primary stress-test case because its
active-space boundary is architecturally most demanding and its structural mutation
class---producing delocalized electronic perturbations across the hydrophobic core of the
DNA-binding domain---places it in the regime where classical density-functional methods
fail at chemical accuracy and fermionic quantum simulation is the principal path to
chemical-accuracy characterisation.
...
This paper does not report a completed full-pocket C275F quantum simulation on physical
