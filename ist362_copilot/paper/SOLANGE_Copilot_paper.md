...
Retrieval **precision** matters as much as recall for auditability: an unrelated passage shown next to an answer reads as false supporting evidence. We therefore remove stopwords and apply light suffix stemming before BM25 scoring (so a query like "How is a run notarized?" matches on *notari-* and *run* rather than on "how"/"is", and irrelevant passages score zero and never appear), and the evidence panel displays **only the passages the answer actually cited** — or, when a smaller model omits brackets, only those within 35% of the top relevance score. The result is that the source panel can never present an unsupporting passage as grounding.
...
1. **Small local models are weaker than frontier cloud models — and fail in different ways.** `qwen3:0.6b` misses ~1 in 7 extraction fields; `deepseek-r1:1.5b` is weakest overall. Two distinct qualitative failures illustrate this. On a single-hop factual question, `deepseek-r1:1.5b` gave a correct, grounded answer and then appended the refusal sentence "that is not in the provided context" — an *over-inclusion* failure, contradicting itself. On a harder, multi-fact synthesis question ("how did KRAS G12C become druggable?", which requires combining the covalent mechanism, the target residue, and the pocket), `qwen3:0.6b` — despite giving a correct answer on the easier question — *refused outright*, an *over-caution* failure. Both are limits of small-model *instruction following*: a language model generates token-by-token without a global plan, so a small model can pattern-match to conflicting instructions ("answer from context" and "refuse if absent") rather than evaluating the condition once, and its output is a stochastic sample that sits closer to the decision boundary than a larger model's. Consequently, neither small model is uniformly worse than the other across every question — reliability is *query-dependent*, not a fixed per-model ranking. This is precisely why the evaluation harness (Section 5) averages over multiple questions and repeated runs rather than trusting any single exchange: a lone transcript is anecdote, not evidence. The larger models (llama3.2, deepseek-r1:8b) handled both questions cleanly. Grounding discipline is therefore *model-dependent*: a good prompt is necessary but not sufficient — the model must be capable enough to follow it consistently. The tool mitigates but does not eliminate this.
