...
1. **Small local models are weaker than frontier cloud models.** `qwen3:0.6b` misses ~1 in 7 extraction fields; `deepseek-r1:1.5b` is weakest overall. A telling qualitative failure: on one question `deepseek-r1:1.5b` gave a correct, grounded answer and then appended the refusal sentence "that is not in the provided context" — contradicting itself. This is a limit of small-model *instruction following*: because a language model generates token-by-token without a global plan, a 1.5B model can pattern-match to both salient instructions ("answer from context" and "refuse if absent") instead of evaluating the condition once, and reasoning models can leak self-correction into the final answer. The larger models (llama3.2, deepseek-r1:8b) handled the same prompt cleanly. Grounding discipline is therefore *model-dependent*: a good prompt is necessary but not sufficient — the model must be capable enough to follow it. The tool mitigates but does not eliminate this.
